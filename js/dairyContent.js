...
function renderItemsInModal(items) {
	...
	const modal = document.getElementById('dairyResultsModal');
...
document.getElementById('searchForm').addEventListener('submit', function(event) {
	...
	const filteredItems= dairyItems.filter(item => item.name.toLowerCase().includes(searchTerm));
	renderItemsInModal(filteredItems);
});

// Adding item to basket
function addToBasket(itemId) {
	const selectedItem = dairyItems.find(item => item.id === itemId);
